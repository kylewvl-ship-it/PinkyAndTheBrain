...
- [x] Task 1: Create `scripts/resolve-findings.ps1` (AC: 1, 2)
  - [x] 1.1 Parameters: `-ReportFile` (path to `knowledge/reviews/health-report-YYYY-MM-DD.md`), `-FindingIndex` (1-based integer to target a specific finding), `-Action` (ValidateSet: `update-metadata`, `accept-extracted`, `reject-extracted`, `fix-link`, `merge-duplicate`, `ignore-fingerprint`, `rebuild-index`, `archive`, `defer`), `-DeferNote` (string, used with `defer`), `-Force` (switch, skip confirmation prompt), `-WhatIf` (switch), `-Help` (switch)
  - [x] 1.2 Parse the report Markdown file to extract the ordered findings list; fail clearly if file not found or malformed
  - [x] 1.3 For `fix-link`: read the affected file, find the broken link, compute Levenshtein distance against all `.md` stems in all non-archived knowledge folders, present top-3 candidates (or none if no close match), prompt for selection or `mark-broken`; apply chosen fix with `Set-Content -Encoding UTF8`
  - [x] 1.4 For `update-metadata`: prompt for the new field value (or accept via `-Value` param), write updated frontmatter using `scripts/lib/frontmatter.ps1` helpers, print before/after diff
  - [x] 1.5 For `archive`: delegate to `scripts/archive-content.ps1 -File <path> -Reason "health-check-resolution"` (do not re-implement archive logic)
  - [x] 1.6 For `rebuild-index`: emit a clear "index.md rebuild is a manual step — edit `<folder>/index.md` to reflect current content" message; this action is informational only
  - [x] 1.7 For `defer`: write a defer record to `.ai/health-deferred.json` (create if absent); record: `{ "file": "...", "rule": "...", "deferred_until": "<ISO-8601 +30 days>", "note": "...", "deferred_at": "<now>" }`; print confirmation
  - [x] 1.8 For `ignore-fingerprint`, `accept-extracted`, `reject-extracted`, `merge-duplicate`: record the decision in `.ai/health-deferred.json` with a special `action` field so the decision persists; emit a clear "recorded — re-run health-check to see updated report" message. Full automated merge is out of scope.
  - [x] 1.9 Confirmation gate: unless `-Force` is set, print the planned change and prompt `Y/N` before applying; in `PINKY_FORCE_NONINTERACTIVE = "1"` mode treat absence of `-Force` as abort with exit `0`

- [x] Task 2: Integrate deferred findings into `scripts/health-check.ps1` (AC: 4)
  - [x] 2.1 At the end of each health check run, load `.ai/health-deferred.json` if present; filter out findings whose `file` + `rule` match an active defer record (where `deferred_until` is in the future)
  - [x] 2.2 Collect all active defer records that match suppressed findings; append them as a "Deferred Issues" section in the console output and in the report file, showing: file, rule, deferred_until, note
  - [x] 2.3 Expired defer records (where `deferred_until` is in the past) are treated as if absent — the finding reappears normally
  - [x] 2.4 Do not modify the `.ai/health-deferred.json` file during a health check run (read-only consumption)

- [x] Task 3: Batch operations via `scripts/resolve-findings.ps1` (AC: 3)
  - [x] 3.1 Add `-Batch` switch: when set with `-Action update-metadata -Field <name> -Value <val>`, apply the metadata update to all findings in the report that have the same `rule` and missing field
  - [x] 3.2 Add `-BulkArchiveStale` switch: archive all files in the report with `Stale Content` severity `High` (>12 months) that haven't been updated in over a year; delegate each to `archive-content.ps1`; prompt once for batch confirmation unless `-Force`
  - [x] 3.3 Add `-BatchExtendReview -Days <n> -Project <tag>`: for all `Stale Content` findings in files whose frontmatter `project` matches `<tag>`, call `update-wiki-metadata.ps1 -File <path> -ReviewTrigger <now+days>`. Emit a summary of files updated.

- [x] Task 4: Pester tests in `tests/resolve-findings.Tests.ps1` (AC: 1–4)
  - [x] 4.1 Setup: `$TestDrive`, `$env:PINKY_VAULT_ROOT`, `$env:PINKY_GIT_REPO_ROOT`, `$env:PINKY_FORCE_NONINTERACTIVE = "1"` — match all existing test patterns
  - [x] 4.2 Test fix-link: file with `[[missing-page]]` → script presents candidates, selects first, link is rewritten and confirmed
  - [x] 4.3 Test update-metadata: file missing `confidence` → script writes `confidence: medium` to frontmatter, before/after printed
  - [x] 4.4 Test archive action: delegates to `archive-content.ps1` with expected args; original file moved/archived
  - [x] 4.5 Test defer: creates `.ai/health-deferred.json` with correct `deferred_until` (+30 days); deferred finding absent from next `health-check.ps1` run
  - [x] 4.6 Test defer expiry: expired defer record causes finding to reappear in health check output
  - [x] 4.7 Test batch update-metadata: all findings with same missing field updated in one run
  - [x] 4.8 Test bulk-archive-stale: High-severity stale files archived; Medium-severity files untouched
  - [x] 4.9 Test confirmation gate: `-Force` skips prompt; absent `-Force` in NONINTERACTIVE mode aborts with exit 0
  - [x] 4.10 Test deferred section in health report: deferred findings appear under "Deferred Issues" in both console and report file

- [x] Task 5: Validate and update story status
  - [x] 5.1 Run `Invoke-Pester tests\resolve-findings.Tests.ps1`
  - [x] 5.2 Run `Invoke-Pester tests\health-check.Tests.ps1` (regression — story 6.1)
  - [x] 5.3 Update Dev Agent Record, File List, and status when complete
...
- `powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Invoke-Pester tests\resolve-findings.Tests.ps1"` — Passed: 9, Failed: 0
- `powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Invoke-Pester tests\health-check.Tests.ps1"` — Passed: 13, Failed: 0
- Direct `Invoke-Pester tests\resolve-findings.Tests.ps1` was blocked by local PowerShell execution policy before loading tests.

### Completion Notes List

- Implemented `scripts/resolve-findings.ps1` with targeted report parsing, confirmation gating, metadata updates through frontmatter helpers, link candidate repair, defer records, decision records, archive delegation, and requested batch operations.
- Integrated read-only active deferral suppression into `scripts/health-check.ps1` and added Deferred Issues output to console and markdown reports.
- Added `health-check-resolution` to archive-content allowed reasons so the required delegated archive call is accepted without duplicating archive logic.
- Added resolver Pester coverage for fix-link, metadata updates, archive delegation, defer/expiry, batch metadata, bulk stale archive, confirmation gate, and deferred report sections.

### File List

- `_bmad-output/implementation-artifacts/6-2-health-check-finding-resolution.md`
- `_bmad-output/implementation-artifacts/sprint-status.yaml`
- `scripts/resolve-findings.ps1`
- `scripts/health-check.ps1`
- `scripts/archive-content.ps1`
- `tests/resolve-findings.Tests.ps1`

## Change Log

- 2026-04-29: Implemented Story 6.2 health-check finding resolution workflow and tests.
